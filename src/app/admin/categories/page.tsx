...
export default function CategoriesPage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="rounded-2xl p-6 mb-6 animate-fade-in transition-all" style={cardStyle}>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 rounded-2xl" style={cardStyle}>
          <FolderOpen size={48} className="mx-auto mb-4 opacity-30" style={{ color: c.muted }} />
          <p className="text-lg" style={{ color: c.muted }}>暂无分类</p>
        </div>
      ) : (
        <div className="space-y-3">
          {categories.map(cat => (
            <div key={cat.id} className="rounded-2xl px-6 py-5 flex items-center justify-between transition-all duration-200 hover:translate-x-1 hover:shadow-lg"
              style={cardStyle}>
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, #f59e0b 0%, #d4a84b 100%)", opacity: 0.8 }}>
                  <FolderOpen size={18} className="text-white" />
                </div>
                <div>
                  <p className="text-base font-medium" style={{ color: c.white }}>{cat.name}</p>
                  <p className="text-xs font-mono mt-1" style={{ color: c.muted }}>/{cat.slug} · {cat._count.posts} 篇文章</p>
                </div>
              </div>
